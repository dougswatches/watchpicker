...
function ValuationTool() {
  ...
  const reset = () => { setFormData({ brand: "", model: "", reference: "", condition: "", notes: "" }); setResult(null); setError(null); setShared(false); };
  ...
  const shareValuation = () => {
    const text = result ? `${result.watch_name} — valued at ${formatGBP(result.value_mid)} (${result.verdict}). Get your free valuation at ${window.location.origin}` : '';
  ...
  if (result) return <ValuationResult result={result} onReset={reset} onTweak={tweakInputs} onShare={shareValuation} shared={shared} formatGBP={formatGBP}/>;
...
function ValuationResult({ result, onReset, onTweak, onShare, shared, formatGBP }) {
  ...
  const comparables = result.comparable_sales || [];
  ...
  return (
    <div style={{maxWidth:600,margin:"0 auto",padding:"1.5rem"}}>
      ...
      <div style={{
        ...
        <div className="mobile-flex-wrap" style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:"1.25rem",flexWrap:"wrap",gap:"0.5rem"}}>
          <p style={{fontSize:"0.68rem",fontWeight:700,letterSpacing:"0.12em",color:"#666",margin:0}}>ESTIMATED MARKET VALUE</p>
          <div style={{display:"flex",gap:"0.4rem",alignItems:"center"}}>
            <span style={{
              fontSize:"0.65rem",fontWeight:700,letterSpacing:"0.1em",
              padding:"0.2rem 0.6rem",borderRadius:2,background:vc.bg,color:vc.text,
            }}>{result.verdict}</span>
            {result.value_retention_pct && (
              <span style={{
                fontSize:"0.6rem",fontWeight:700,letterSpacing:"0.06em",
                padding:"0.2rem 0.5rem",borderRadius:2,
                background:result.value_retention_pct >= 90 ? "#2d6a4f" : result.value_retention_pct >= 70 ? "#555" : "#c0392b",
                color:"#fff",
              }}>RETAINS {result.value_retention_pct}%</span>
            )}
          </div>
        </div>
        ...
        <div className="mobile-flex-wrap" style={{display:"flex",gap:"1.5rem",marginBottom:"1rem",flexWrap:"wrap"}}>
          <div>
          ...
          {result.price_2_years_ago && (
            <div>
              <p style={{fontSize:"0.62rem",fontWeight:700,letterSpacing:"0.1em",color:"#555",marginBottom:"0.15rem"}}>2 YRS AGO</p>
              <div style={{display:"flex",alignItems:"center",gap:"0.35rem"}}>
                <p style={{fontSize:"1rem",fontWeight:600,color:"#999",margin:0}}>{formatGBP(result.price_2_years_ago)}</p>
                {result.price_change_pct != null && (
                  <span style={{
                    fontSize:"0.65rem",fontWeight:700,
                    color: result.price_change_pct >= 0 ? "#2d6a4f" : "#c0392b",
                  }}>{result.price_change_pct >= 0 ? "+" : ""}{result.price_change_pct}%</span>
                )}
              </div>
            </div>
          )}
        ...
        <div className="mobile-flex-wrap" style={{
          ...
          <span style={{fontSize:"0.78rem",color:"#999",fontWeight:500}}>Market {result.market_trend}</span>
          {result.market_trend_note && (
            <span style={{fontSize:"0.75rem",color:"#666",fontWeight:400}}>— {result.market_trend_note}</span>
          )}
        </div>
      </div>

      {/* Price History */}
      {result.price_history_note && (
        <div style={{
          ...
          <p style={{fontSize:"0.68rem",fontWeight:700,letterSpacing:"0.1em",color:"#888",marginBottom:"0.6rem"}}>PRICE HISTORY</p>
          <p style={{fontSize:"0.875rem",lineHeight:1.7,color:"#444",margin:0}}>{result.price_history_note}</p>
        </div>
      )}
      ...
      <div style={{
        ...
        <p style={{fontSize:"0.875rem",lineHeight:1.7,color:"#444",margin:0}}>{result.verdict_reason}</p>
      </div>

      {/* Suggested Listing Price — only if SELL */}
      {result.suggested_listing_price && (
        <div style={{
          background:"#fff",border:"1px solid #c0392b",borderRadius:4,padding:"1.25rem",
          marginBottom:"1rem",animation:"fadeUp 0.4s ease both",animationDelay:"0.12s",
        }}>
          <p style={{fontSize:"0.68rem",fontWeight:700,letterSpacing:"0.1em",color:"#c0392b",marginBottom:"0.5rem"}}>SUGGESTED LISTING PRICE</p>
          <p style={{fontSize:"1.5rem",fontWeight:700,color:"#1a1a1a",margin:"0 0 0.35rem",lineHeight:1}}>{formatGBP(result.suggested_listing_price)}</p>
          {result.listing_price_note && (
            <p style={{fontSize:"0.8rem",lineHeight:1.6,color:"#666",margin:0}}>{result.listing_price_note}</p>
          )}
        </div>
      )}
      ...
      {comparables.length > 0 && (
        <div style={{
          ...
          <p style={{fontSize:"0.68rem",fontWeight:700,letterSpacing:"0.1em",color:"#888",marginBottom:"0.75rem"}}>RECENT COMPARABLE SALES</p>
          <div style={{display:"flex",flexDirection:"column",gap:"0.65rem"}}>
            {comparables.map((comp, i) => (
              <div key={i} style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:"0.75rem"}}>
                <div style={{flex:1,minWidth:0}}>
                  <p style={{fontSize:"0.82rem",color:"#333",margin:0,fontWeight:500}}>{comp.description}</p>
                  <p style={{fontSize:"0.72rem",color:"#aaa",margin:"0.1rem 0 0"}}>{comp.source}</p>
                </div>
                <span style={{fontSize:"0.95rem",fontWeight:700,color:"#1a1a1a",flexShrink:0}}>{formatGBP(comp.price)}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      {sellLinks.length > 0 && (
        <div style={{
          background:"#fff",border:"1px solid #e8e8e8",borderRadius:4,padding:"1.25rem",
          marginBottom:"1rem",animation:"fadeUp 0.4s ease both",animationDelay:"0.4s",
        }}>
      ...
      <div className="mobile-flex-wrap" style={{display:"flex",gap:"0.6rem",alignItems:"center",flexWrap:"wrap"}}>
        ...
        }}>{shared ? "COPIED ✓" : "SHARE VALUATION"}</button>
      </div>
    </div>
  );
}
